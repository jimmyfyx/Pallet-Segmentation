...
class HeadDepth(nn.Module):
    def __init__(self, features, outputs):
        ...
        self.head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(32, outputs, kernel_size=1, stride=1, padding=0),
        )
...
class HeadDepthClassi(nn.Module):
    def __init__(self, features, outputs):
        super(HeadDepthClassi, self).__init__()
        self.head = nn.Sequential(
            nn.Conv2d(features, features // 2, kernel_size=3, stride=1, padding=1),
            ...
            nn.ReLU(),
            # Final layers
            nn.Conv2d(features // 8, features // 16, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            nn.Conv2d(features // 16, outputs, kernel_size=1, stride=1),
        )
